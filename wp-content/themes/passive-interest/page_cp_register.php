...
if ($http_post) {
	if ($_GET['action'] == 'resend') {
		...
		$errors = gs_register_new_user($user_login, $user_email);
...
function gs_register_new_user( $user_login, $user_email ) {
	$errors = new WP_Error();
	...
	<div class="row-fluid">
		...
		<div class="span4 usercp-wrapper">
			<?php if ($_GET['action'] == 'resend') { ?>
				...
				<div class="error-msg"><div class="alert alert-success"><strong><?php _e('Please check your email for activation.', 'ipin'); ?></strong></div></div>
				...
				<form id="resendform" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
					...
					<input type="submit" class="btn btn-large btn-primary" name="wp-submit" id="wp-submit" value="<?php _e('Resend', 'ipin'); ?>" />
	
					<br /></br />
					<?php _e('Check your junk/spam folder if you did not receive the activation email', 'ipin'); ?>
				</form>
			<?php } else { ?>
				<h1><?php _e('Sign up', 'ipin') ?></h1>
				...
				<form name="registerform" id="registerform" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
					...
					<p class="comment-form-anti-spam" style="clear:both;">
						<label for="anti-spam">Current year <span class="required">*</span>
					...
					<?php if (of_get_option('register_agree') != '0') { ?>
						...
						<a onClick="window.open('<?php echo get_permalink(of_get_option('register_agree')); ?>','','resizable=1,scrollbars=1,top=0,left=0,width=640,height=480')" href="<?php echo get_permalink(of_get_option('register_agree')); ?>" target="_blank">
							<strong><?php _e('Terms of Service', 'ipin'); ?></strong>
						</a>
						</p>
					<?php } ?>
					<br  />
					<input<?php if (of_get_option('register_agree') != '0') echo ' disabled="disabled"'; ?> type="submit" class="btn btn-large btn-primary" name="wp-submit" id="wp-submit" value="<?php _e('Sign up', 'ipin'); ?>" tabindex="50" />
                    
                    <br />
                    
        <?php do_shortcode('[GSFBLOGIN class="first_fb_signup_gs fb_reg" text="Sign up with Facebook"]'); ?>
    	<?php do_shortcode('[GSTWLOGIN class="first_tw_signup_gs tw_reg" text="Sign up with Twitter"]'); ?>
