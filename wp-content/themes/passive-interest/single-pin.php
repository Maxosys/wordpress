					...
					<div id="post-<?php the_ID(); ?>" <?php post_class('post-wrapper'); ?>>
						<div class="post-top-wrapper">
							...
							<div class="post-top-wrapper-header">
								...
								<button class="btn pull-right follow ipin-follow<?php $data_boards = get_the_terms($post->ID, 'board'); foreach ($data_boards as $data_board) { $board_parent_id = $data_board->parent; $board_id = $data_board->term_id; } if ($followed = ipin_followed($board_parent_id)) { echo ' disabled'; } ?>" data-board_parent_id="0" data-author_id="<?php echo $post->post_author; ?>" data-board_id="<?php echo $board_parent_id; ?>" type="button"><?php if (!$followed) { _e('Follow', 'ipin'); } else { _e('Unfollow', 'ipin'); } ?></button>
								<?php } ?>
								<a href="<?php echo home_url('/' . $wp_rewrite->author_base . '/') . get_the_author_meta('user_nicename'); ?>/">
						...
						<div class="post-share">
							<p><iframe src="//www.facebook.com/plugins/like.php?href=<?php echo rawurlencode(get_permalink()); ?>&amp;send=false&amp;layout=button_count&amp;width=75&amp;show_faces=false&amp;action=like&amp;colorscheme=light&amp;font&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:75px; height:21px;" allowTransparency="true"></iframe></p>

							<p><a href="https://twitter.com/share" class="twitter-share-button" data-url="<?php the_permalink(); ?>" data-text="<?php echo preg_replace('/[\n\r]/', ' ', the_title_attribute('echo=0')); ?>">Tweet</a></p>

							<p><div class="g-plusone" data-size="small" data-href="<?php the_permalink(); ?>"></div></p>
							
							<p><a data-pin-config="beside" href="//pinterest.com/pin/create/button/?url=<?php echo rawurlencode(get_permalink()); ?>&media=<?php echo rawurlencode($imgsrc[0]); ?>&description=<?php echo preg_replace('/[\n\r]/', ' ', the_title_attribute('echo=0')); ?>" data-pin-do="buttonPin"><img border="0" src="//assets.pinterest.com/images/pidgets/pin_it_button.png" /></a></p>
							...
							<p><a id="post-report" class="btn btn-mini"><strong><i class="icon-flag"></i> <?php _e('Report', 'ipin'); ?></strong></a></p>
						</div>
						...
						<div class="post-top-meta">
							<div class="pull-left">
								<div class="post-actionbar">
									...
									<a class="ipin-repin btn" data-post_id="<?php echo $post->ID ?>" href="#"><i class="icon-pushpin"></i> <?php _e('Repin', 'ipin'); ?></a>
									<?php } ?>
									<?php if ($post->post_author != $user_ID) { ?> 
									<button class="ipin-like btn <?php if (ipin_liked($post->ID)) { echo ' disabled'; } ?>" data-post_id="<?php echo $post->ID ?>" data-post_author="<?php echo $post->post_author; ?>" type="button"><i class="icon-heart"></i> <?php _e('Like', 'ipin'); ?></button>
									<?php } if ($post->post_author == $user_ID || current_user_can('edit_others_posts')) { ?>
									<a class="btn" href="<?php echo home_url('/pins-settings/'); ?>?i=<?php the_ID(); ?>"><?php _e('Edit', 'ipin'); ?></a>
									<?php } ?>
									...
									if ($is_video == 'no') {
									?>
									<a class="ipin-zoom btn" href="<?php echo $imgsrc_full; ?>"><i class="icon-zoom-in"></i> <?php _e('Zoom', 'ipin'); ?></a>
									<?php } ?>
								</div>
							</div>
						...
						if ($is_video == 'youtube') {
						?>
							<embed id="video-embed" src="http://www.youtube.com/v/<?php echo $videomatch[1]; ?>?rel=0&autoplay=1" type="application/x-shockwave-flash" width="520" height="292" allowscriptaccess="always" allowfullscreen="true" wmode="opaque"></embed>
							<img class="featured-thumb hide" src="<?php echo $imgsrc[0]; ?>" alt="<?php the_title_attribute(); ?>" />
						<?php
						//if is vimeo video
						} else if ($is_video == 'vimeo') {
						?>
							<iframe id="video-embed" src="http://player.vimeo.com/video/<?php echo $video_id; ?>?title=0&amp;byline=0&amp;portrait=0&amp;badge=0&amp;color=ffffff&amp;autoplay=1" width="520" height="292" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
							<img class="featured-thumb hide" src="<?php echo $imgsrc[0]; ?>" alt="<?php the_title_attribute(); ?>" />
						<?php
						} else {
						?>
						...
						<?php } ?>
							...
							<div class="post-share-horizontal visible-phone">
								<iframe src="//www.facebook.com/plugins/like.php?href=<?php echo rawurlencode(get_permalink()); ?>&amp;send=false&amp;layout=button_count&amp;width=75&amp;show_faces=false&amp;action=like&amp;colorscheme=light&amp;font&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:75px; height:21px;" allowTransparency="true"></iframe>
								...
								<a id="post-report" class="btn btn-mini"><strong><i class="icon-flag"></i> <?php _e('Report', 'ipin'); ?></strong></a>
							</div>
						</div>
						...
						<div class="post-content">
							<?php if (of_get_option('form_title_desc') != 'separate') { ?>						
								<?php if (mb_strlen(get_the_title()) < 120) { ?>
									<h1 class="post-title" data-title="<?php esc_attr_e($post->post_title); ?>" data-tags="<?php esc_attr_e(substr($tags, 0, -2)); ?>" data-price="<?php esc_attr_e($post_price); ?>" data-content="<?php esc_attr_e($post->post_content); ?>"><?php echo wpautop(preg_replace_callback('/<a[^>]+/', 'ipin_nofollow_callback', get_the_title())); ?></h1>
								<?php } else { ?>
									<div class="post-title" data-title="<?php esc_attr_e($post->post_title); ?>" data-tags="<?php esc_attr_e(substr($tags, 0, -2)); ?>" data-price="<?php esc_attr_e($post_price); ?>" data-content="<?php esc_attr_e($post->post_content); ?>"><?php echo wpautop(preg_replace_callback('/<a[^>]+/', 'ipin_nofollow_callback', get_the_title())); ?></div>
								<?php } ?>
							<?php } else { ?>
									<h1 class="post-title post-title-large" data-title="<?php esc_attr_e($post->post_title); ?>" data-tags="<?php esc_attr_e(substr($tags, 0, -2)); ?>" data-price="<?php esc_attr_e($post_price); ?>" data-content="<?php esc_attr_e($post->post_content); ?>"><?php the_title(); ?></h1>
							<?php } ?>
						...
						<div class="post-likes">
							<div class="post-likes-wrapper">
								...
								foreach ($post_likes[0] as $post_like) {
									...
									?>
									<a id="likes-<?php echo $post_like; ?>" href="<?php echo home_url('/' . $wp_rewrite->author_base . '/') . $like_author->user_nicename; ?>/" rel="tooltip" title="<?php esc_attr_e($like_author->display_name); ?>">
