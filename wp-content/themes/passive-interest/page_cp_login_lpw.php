...
if ($_GET['action'] == 'rp') {
	$user = gs_check_password_reset_key($_GET['key'], $_GET['login']);
...
if ($_POST['action'] == 'rp') {
	...
	if ($_POST['pass1'] != $_POST['pass2'] ) {
		...
		gs_reset_password($user, $_POST['pass1']);
...
//function from wp-login.php
function gs_check_password_reset_key($key, $login) {
	global $wpdb;

	$key = preg_replace('/[^a-z0-9]/i', '', $key);

	if ( empty( $key ) || !is_string( $key ) )
		return new WP_Error('invalid_key', __('Invalid key', 'ipin'));
...
//function from wp-login.php
function gs_reset_password($user, $new_pass) {
