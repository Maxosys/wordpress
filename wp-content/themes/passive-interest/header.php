...
<head>
    ...
    if(is_page('register'))
	{
		echo 'Sign up | '.get_bloginfo( 'description', 'display' );
	}
	else
	{
		wp_title( '', true, 'right' );
			...
			<div class="container">
				...
				<nav id="nav-main" class="nav-collapse" role="navigation">
					...
					<?php if (is_user_logged_in()) { global $user_ID, $user_identity; ?>
						...
						<li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" data-target="" href=""><?php _e('Add', 'ipin'); ?> <i class="icon-caret-down"></i></a>
						...
						<li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" data-target="" href=""><?php echo $user_identity; ?> <i class="icon-caret-down"></i></a>
...
<?php 
  ...
  {
   ?>
  	<li><a href="<?php echo site_url().'/analytics?analyticview=analyticsmetrics'; ?>"><?php _e('Analytics', 'ipin'); ?></a></li>
 <?php  
  }
  else 
  {
   ?>
		<li> <a href="<?php echo site_url().'/analytics?analyticview=verification'; ?>"><?php _e('Analytics', 'ipin'); ?></a> </li>
  <?php
  }
 ?>
                                
								<li><a href="<?php echo home_url('/settings/'); ?>"><?php _e('Settings', 'ipin'); ?></a></li>
                                <li><a href="<?php echo site_url() . '/setup-wizard'; ?>"><?php _e('Setup Wizard', 'ipin'); ?></a></li>
                                <li><a href="<?php echo 'http://support.piformula.com'; ?>"><?php _e('Support', 'ipin'); ?></a></li>
                                <hr>
				...
				<?php   } else { ?>
						...
						<li class="visible-desktop" id="loginbox-wrapper"><a id="loginbox" data-content='<?php if (function_exists('wsl_activate')) { do_action('wordpress_social_login'); echo '<hr />'; } ?>' 
							 aria-hidden="true"><i class="icon-signin"></i> <?php _e('Register / Login', 'ipin'); ?></a>
						</li>
					<?php } ?>
					</ul>
					...
					<a href="<?php echo $facebook_icon_url; ?>" title="<?php _e('Find us on Facebook', 'ipin'); ?>" class="topmenu-social"><i class="icon-facebook"></i></a>
					<?php } ?>

					<?php if ('' != $twitter_icon_url = of_get_option('twitter_icon_url')) { ?>
					<a href="<?php echo $twitter_icon_url; ?>" title="<?php _e('Follow us on Twitter', 'ipin'); ?>" class="topmenu-social"><i class="icon-twitter"></i></a>
					<?php } ?>

					<a href="<?php bloginfo('rss2_url'); ?>" title="<?php _e('Subscribe to our RSS Feed', 'ipin'); ?>" class="topmenu-social"><i class="icon-rss"></i></a>
	...
	<?php if (!$user_ID && of_get_option('top_message') != '' && !is_page('register') && !is_page('login')) { ?>	
	<div id="top-message-wrapper" class="container">
		<div class="row">
			<div class="span3 hidden-phone"></div>
			<div id="top-message" class="span6">
				<p class="pull-right">
					<a class="btn btn-small btn-primary" href="<?php echo home_url('/register/');?>"><?php _e('Sign up', 'ipin'); ?></a>
					<a class="btn btn-small" href="<?php echo home_url('/login/');?>"><?php _e('Login', 'ipin'); ?></a>

					<?php do_shortcode('[GSFBLOGIN class="first_fb_signup_gs fb_reg_msg" text="Login"]'); ?> 
                    <?php do_shortcode('[GSTWLOGIN class="first_tw_signup_gs tw_reg_msg" text="Login"]'); ?>
