		...
		<div class="span4 user-wrapper">
			<div class="post-content">
				...
				<div class="user-profile">
                    ...
                    <p class="pif_profile_member_id"><?php if (strlen(get_user_meta($user_info->ID,"MEMBERID", true)) > 0) { ?>
					<a href="<?php printf(WIZARD_LINK_FOR_MEMBERID, get_user_meta($user_info->ID,"MEMBERID", true)); ?>" target="_blank">Register with me</a> 
					<?php } ?></p>

					<?php if ($user_info->user_url) { ?>
					<a href="<?php echo esc_url($user_info->user_url); ?>" target="_blank"><i class="icon-globe"></i></a> 
					<?php } ?>

					<?php if ($user_info->ipin_user_facebook) { ?>
					<a href="http://www.facebook.com/<?php echo esc_attr($user_info->ipin_user_facebook); ?>" target="_blank"><i class="icon-facebook-sign"></i></a> 
					<?php } ?>

					<?php if ($user_info->ipin_user_twitter) { ?>
					<a href="http://twitter.com/<?php echo esc_attr($user_info->ipin_user_twitter); ?>" target="_blank"><i class="icon-twitter"></i></a> 
					<?php } ?>

					<?php if ($user_info->ipin_user_pinterest) { ?>
					<a href="http://pinterest.com/<?php echo esc_attr($user_info->ipin_user_pinterest); ?>" target="_blank"><i class="icon-pinterest"></i></a> 
					<?php } ?>

					<?php if ($user_info->ipin_user_googleplus) { ?>
					<a href="http://plus.google.com/<?php echo esc_attr($user_info->ipin_user_googleplus); ?>" target="_blank"><i class="icon-google-plus"></i></a> 
					<?php } ?>

					<?php if ($user_info->ipin_user_location) { ?>
					<a href="http://maps.google.com/?q=<?php echo esc_attr($user_info->ipin_user_location); ?>" target="_blank"><i class="icon-map-marker"></i> <small><?php echo esc_attr($user_info->ipin_user_location); ?></small></a> 
					<?php } ?>
				</div>
		...
		<div id="userbar" class="navbar">
			<div class="navbar-inner">
				<ul class="nav">
					...
					<li<?php if ($_GET['view'] == 'pins') { echo ' class="active"'; } ?>><a href="<?php echo get_author_posts_url($user_info->ID); ?>?view=pins"><strong><?php echo $pins_count; ?></strong> <?php if ($pins_count == 1) { _e('Pin', 'ipin'); } else { _e('Pins', 'ipin'); } ?></a></li>
					...
					<li<?php if ($_GET['view'] == 'followers') { echo ' class="active"'; } ?>><a href="<?php echo get_author_posts_url($user_info->ID); ?>?view=followers"><strong id="ajax-follower-count"><?php echo $followers_count; ?></strong> <?php if ($followers_count == 1) { _e('Follower', 'ipin'); } else { _e('Followers', 'ipin'); } ?></a></li>
					...
					<?php if ($user_info->ID != $user_ID) {	?>
						<button class="btn follow ipin-follow<?php if ($followed = ipin_followed($parent_board['term_id'])) { echo ' disabled'; } ?>" data-author_id="<?php echo $user_info->ID ?>" data-board_id="<?php echo $parent_board['term_id'];  ?>" data-board_parent_id="<?php echo $parent_board['parent']; ?>" data-disable_others="no" type="button"><?php if (!$followed) { _e('Follow', 'ipin'); } else { _e('Unfollow', 'ipin'); } ?></button>
					<?php } else { ?>
						<a class="btn follow" href="<?php echo home_url('/settings/'); ?>"><strong><?php _e('Edit Profile', 'ipin'); ?></strong></a>
					<?php } ?>
					</li>
		...
		foreach ($followers[0] as $follower) {
			...
			<div class="follow-wrapper">
				...
				<?php } ?>
					...
					<div class="user-name">
						<h4><a href="<?php echo home_url('/' . $wp_rewrite->author_base . '/') . $follower_info->user_nicename; ?>/"><?php echo $follower_info->display_name; ?></a></h4>
					</div>
				</div>
			</div>
		<?php 
		...
		foreach ($boards_paginated as $board) {
			...
			<div class="board-mini">
				...
				<div class="board-photo-frame">
					...
					// $post_array contains links with all photos. Can be used to select one of them that the user wants to display. The only thing left is to make a pattern of how the user can select it
