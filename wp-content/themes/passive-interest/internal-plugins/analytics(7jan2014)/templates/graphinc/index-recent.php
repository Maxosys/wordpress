<?php
	...
	//check user domain in post meta for https, http and www code starts here
	
	
	$userdomain =  get_user_meta($user_id, 'website_url', true); // get user website according to user id

	$ddurl = explode('://',$userdomain); // check website for http,https,www
	...
	$userdomain = rtrim($make_url,'.');  // yourdomain.com
	...
	// fetch data from postmeta according to the user domain 
	
	$query = "SELECT * FROM $wpdb->postmeta WHERE meta_value like '%".$userdomain."%' and meta_key='_Photo Source' ";
	$post_meta_details = $wpdb->get_results($query);
	
	
	
	
	// extract postids from the above array
	...
	$uni_data = array_unique($postids);   // fetch unique post ids
	
	$validpostids = array_values($uni_data); // re-index array
	...
	for($z=0;$z<count($post_data);$z++) {
		$pid[]=$post_data[$z][0]->ID;
	}	
	
	
	$upostids_first = $pid;  // post ids

	//print_r($upostids_first);		// first array all data
	...
	for($j=0;$j<count($upostids_first);$j++) {
		...
		$qq = "select * from wp_postmeta where meta_key='_Repin Post ID' and post_id='".$upostids_first[$j]."' ";
		
		$repinarr_val = $wpdb->get_results($qq);
	...
	for($j=0;$j<count($result_new);$j++) {
		 ...
		 $qq = "select * from wp_postmeta where meta_key='_Original Post ID' and post_id='".$result_new[$j]."' ";
	...
	for($s=0;$s<count($upostids);$s++) {
		$qqq = "SELECT * FROM $wpdb->posts WHERE `ID` = '".$upostids[$s]."' ";
