...
// Most Repinned Piformula Analytics
...
if(!empty($userdomain)):
$query = "SELECT * FROM `wp_postmeta` WHERE  `meta_key` = '_Photo Source Domain' AND  `meta_value` like '%".$userdomain."%'
	";
$postsdetails = $wpdb->get_results($query);
endif;
...
{
	//$qqq = "SELECT *  FROM wp_postmeta WHERE `post_id` = '".$postids[$i]."' AND `meta_key` = '_Original Post ID' AND meta_value!='deleted' ";

	$qqq = "SELECT *  FROM wp_postmeta WHERE `post_id` = '".$postids[$i]."' AND `meta_key` = '_Repin Post ID' AND meta_value!='deleted' ";

	$most_repin_piostIds[] = $wpdb->get_results($qqq);

}
$bbx = array_filter($most_repin_piostIds);

$correct = array_values($bbx);
$ppids=array();
for($s=0; $s<count($correct);$s++)
{

	$ppids[] = 	unserialize($correct[$s][0]->meta_value);
}



$postidf = array();
for($w=0;$w<count($ppids);$w++)
{

	for($t=0;$t<count($ppids[$w]);$t++)
	{
		$postidf[] =$ppids[$w][$t];
	}
}

$uniqueoriginal_postIds = array_filter(array_values($postidf));
$original_postIds = array_unique($uniqueoriginal_postIds);

endif;

$final=array();
if(!empty($original_postIds)) {
	for($d=0;$d<count($original_postIds);$d++)
	{
		$customdate = '2013-12-23';
		//	 $hhhq = "SELECT *  FROM wp_posts WHERE `ID` = '".$original_postIds[$d]."' and post_date like '".$customdate."%' ";
		$hhhq = "SELECT *  FROM wp_posts WHERE `ID` = '".$original_postIds[$d]."'  ";

		$final[] = $wpdb->get_results($hhhq);
...
<div
	...
	<div id="post-<?php the_ID(); ?>" <?php post_class('thumb'); ?>>
		...
		<!-- display user display name according to user id and display their board name-->
		...
		<div class="masonry-meta">
			...
			<div class="masonry-meta-comment">
				...
				$original_post_id = get_post_meta($post->ID, "_Original Post ID");
...
<div
	class="modal hide" id="post-lightbox" tabindex="-1" role="dialog"
