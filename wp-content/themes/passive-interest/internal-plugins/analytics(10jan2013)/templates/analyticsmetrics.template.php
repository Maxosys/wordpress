...
else  
{
	...
	$datearr[] = $date = date('Y-m-d');			// current date 
	for($i=1 ;$i<=15 ;$i++)
	{
		$date1 = str_replace('-', '/', $date);
		$tomorrow = date('Y-m-d',strtotime($date1 . "-$i days"));
		$datearr[] = $tomorrow;			//  days Array
	}
	$lastarrkey = $i-2;	
	...
	$date_value = $f_date2_f.' - '.$f_date2_f2;		// date variable for showing in Input Box
