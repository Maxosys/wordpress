<?php
global $wpdb;
  $key = trim('url_verifed');
  $user_ID = get_current_user_id(); 
  $single = true;
  $isverified = get_user_meta($user_ID,'url_verifed',true); 
 
  if($isverified=="yes") {  ?>
  <script>window.location.href="<?php echo site_url(); ?>/analytics?analyticview=analyticsmetrics";</script>
  <?php   }    ?>
...
function formsubmitwebverify(uid)
{
	...
	if(domainurlname!='')
	{
		...
		$.post("<?php echo bloginfo('template_url');  ?>/internal-plugins/analytics/ajax/VerifiedWebSiteAccordingUser.php/",{uid:uid,domainurlname:domainurlname}).done(function(respurl)
			{	// website verified according to user
				...
				{			
					...
					$.post("<?php echo bloginfo('template_url');  ?>/internal-plugins/analytics/ajax/verifywebsiteformsubmit.php/",{uid:uid,contype:domainurlname}).done(function(resp)
					{
						$("#responseerro").html(" ");
						var res = resp.split(",");
						res[0];
				...
				// set tag value in input box
						$("#metatagrespid").val(res[4]);

						$("#htmlfilename").val(res[0]);
					var url = "<?php echo bloginfo('template_url'); ?>/internal-plugins/analytics/download.php?filename="+res[0];	
...
function isValidURL(url){
	...
	function checkisverified(uid)
	{
		...
		$.post("<?php echo bloginfo('template_url');  ?>/internal-plugins/analytics/ajax/isverifiedwebsite.php/",{uid:uid,verifilename:verficationfilename,websitelink:websiteurl,metatag_type_id:metatag_type_id},function(result){
...
<input type="hidden" id="metatag_type_id" value="upload_type" />
 ...
 <div class="edit">
	...
	<span id="showlink">
		<?php
			include("graphinc/downloadindex.php");	
		?>	 
		<br/>
		Upload this file (<span id="filenamenameidval"> </span>) to <span id="webnamenameidval"> </span>
	</span>
	<span id="hidelink" style="display:none">
		 <input type="text" size="25" id="metatagrespid" value='<meta name="p:domain_verify" content="" />'  />
		 <br/>
