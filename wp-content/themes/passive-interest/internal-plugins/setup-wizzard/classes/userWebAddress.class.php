...
class userWebAddress {
	...
	function set_address($address) {
		...
		if($this->check_address($address)) {
			...
			if($sql) {
				$msg["code"] = 10;
				$msg["success"]["text"] = '<div id="gsSupportTicketSubmitted" class="alert alert-success">Updated all settings successfully.</div>';	
	...
	function check_address($address) {
		...
		$user = new WP_User($curr_user);
		
		if($address == $user->user_nicename) {
			return false;	
		}
