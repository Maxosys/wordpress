...
<a href="<?php echo bloginfo('siteurl');?>/analytics/?analyticview=mostrepinned" class="current">Most Repinned</a>
         ...
         <li>
    <form action="<?php echo $_SERVER['REQUEST_URI'];?>" method="post" id="export_repinned" class="export_repinned" >
    <input type="hidden" name="action" value="exportmostrepinnedcsv" />
    <a href="javascript:;" onclick="document.getElementById('export_repinned').submit();" id="mostrecentpins" class="mostrecentpins">Export</a>
    </form>
    </li>
...
if(!empty($_REQUEST['action']) && $_REQUEST['action']=="exportmostrepinnedcsv") { 
...
$csvpath =  $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/passive-interest/internal-plugins/analytics/csvfiles/';
	...
	$file = @fopen($csvpath."mostrepinned-".date('Y-m-d').".csv","w"); // OPEN CSV FILE FOR WRITE DATA
	
	$fname="mostrepinned-".date('Y-m-d').".csv";
	
	/***************************** LOOP THROUGH PUT EACH PIN IN CSV DOCUMENT ***************************************/ 
	
	
	if(!empty($rps)) {	
	foreach($rps as $k => $repinnedcsv) {
...
/**************************  INITIALLY SET CSV FILE COULMN HEADING  *******************************/
		...
		$post_permalink = $repinnedcsv->guid; // MOST REPINNED LINK
		$imagelink = get_post_meta($repinnedcsv->ID,'_Photo Source',true); // REPINNED LINK 
		$likes_number = get_post_meta($repinnedcsv->ID, '_Likes Count', true); // NUMBER OF LIKES ON REPINNED
		if(empty($likes_number)) { 
			$likes_number=0;
		}
		
		$repins_number = get_post_meta($repinnedcsv->ID, '_Repin Count', true); // NUMBER OF REPINNED
		if(empty($repins_number)) { 
			$repins_number=0;
		}
		$comments_number = get_comments_number(); // NUMBER OF COMMENTS ON REPINNED
		if(empty($comments_number)) { 
			$comments_number=0;
		}
		
		
/**************************  CREATE MOST REPINNED CSV ***************************/
