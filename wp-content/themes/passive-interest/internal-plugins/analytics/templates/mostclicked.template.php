...
<script src="<?php echo bloginfo('template_directory');?>/js/calendar/jscal2.js"></script>
	...
	if(!empty($_GET['date'])) {
			...
			$seldate= $dd1_y.'-'.$dd1_m.'-'.$dd1_d;
			 $f_date2_str = strtotime($seldate);
		$f_date2_f =  date('M d, Y',$f_date2_str);	 
    ?>
    <input size="30" id="f_date1" class="f_date1class" value="<?php echo $f_date2_f;  ?>"  />	<br />
    <?php } 
	else {
		?>
          <input size="30" id="f_date1" class="f_date1class" value="<?php echo date('M d, Y');  ?>"  />	<br />
        <?php 
       }
		?>
...
<li>
<a href="<?php echo bloginfo('siteurl');?>/analytics/?analyticview=mostrepinned" >Most Repinned</a>
</li>
<li><a href="<?php echo bloginfo('siteurl');?>/analytics/?analyticview=mostclicked" class="current">Most Clicked</a></li>
			
    
        <li>
    <form action="<?php echo $_SERVER['REQUEST_URI'];?>" method="post" id="export_clicked" class="export_clicked" >
    <input type="hidden" name="action" value="exportmostclickedcsv" />
    <a href="javascript:;" onclick="document.getElementById('export_clicked').submit();" id="mostrecentpins" class="mostrecentpins">Export</a>
    </form>
    </li>
...
width:500px;
		...
		<?php 
		$mostclicked = new MostClicked();
		$clicked  =  $mostclicked->view_most_clicked();
		if(!empty($clicked[0])) {
		$finalresult = array_filter($clicked);
...
if(!empty($_REQUEST['action']) && $_REQUEST['action']=="exportmostclickedcsv") { 
...
$csvpath =  $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/passive-interest/internal-plugins/analytics/csvfiles/';
	...
	$file = @fopen($csvpath."mostclicked-".date('Y-m-d').".csv","w"); // OPEN CSV FILE FOR WRITE DATA
	
	$fname="mostclicked-".date('Y-m-d').".csv";
	
	/***************************** LOOP THROUGH PUT EACH PIN IN CSV DOCUMENT ***************************************/ 
	if(!empty($rps)) {	
	foreach($rps as $k => $mostclicked) {
		
	/**************************  INITIALLY SET CSV FILE COLUMN HEADING  *******************************/
...
/**************************  INITIALLY SET CSV FILE COULMN HEADING  *******************************/
		...
		$post_permalink = $mostclicked->guid; // MOST CLICKED LINK
		$imagelink = get_post_meta($mostclicked->ID,'_Photo Source',true); // CLICKED LINK 
		$likes_number = get_post_meta($mostclicked->ID, '_Likes Count', true); // NUMBER OF LIKES ON CLIKED
		if(empty($likes_number)) { 
			$likes_number=0;
		}
		
		$repins_number = get_post_meta($mostclicked->ID, '_Repin Count', true); // NUMBER OF CLICKED
		if(empty($repins_number)) { 
			$repins_number=0;
		}
		$comments_number = get_comments_number(); // NUMBER OF COMMENTS ON CLICKED
		if(empty($comments_number)) { 
			$comments_number=0;
		}
		
		
/**************************  CREATE MOST CLICKED CSV ***************************/
