...
jQuery(function($) {
	...
	$('.settings-menu a').click(function(e){
		...
		$('#settingsform').attr('action' , '/settings/')
		var action = $('#settingsform').attr('action') + '#' + page;
		$('#settingsform').attr('action' , action);
		
		e.preventDefault();
	});
	
	$('.sm-mobile').bind('click change' , function(e){
		var page = $(this).val();
...
<div class="container-fluid">
	<div class="row-fluid">
		...
		<div class="span4 usercp-wrapper gs_settings">
<!-------------------------------------------------------------------------------------------------> 
		...
		<ul class="settings-menu">
            ...
            <li><a href="#email_notifications">Email Notifications</a></li>        
        </ul>
        
        <div class="sm-mobile-wrapper">
            <select class="sm-mobile">
                <option selected="selected" value="settings">Settings</option>
                <option value="profile">Profile</option>
                <option value="email_notifications">Email Notifications</option>
            </select>
        </div>
        
		<div id="option-1" class="settings active">
			...
			<form name="settingsform" id="settingsform" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" enctype="multipart/form-data">
				...
				<?php } ?>
					...
					<?php if ($wsl_email_example !== false) { ?>
						<?php echo __('Invalid email provided by') . ' ' . $user_info->wsl_user . '. ' . __('Please enter a valid email to receive email notifications.', 'ipin'); ?>
					<?php } ?>
...
<!------------------------------------------------------------------------------------------------->                  
		...
		<div id="opstion-3" class="email_notifications">
...
<!------------------------------------------------------------------------------------------------->			
			<!-- LUAT DE AICI AVATAR FORM -->
			<form name="avatarform" id="avatarform" class="" method="post" enctype="multipart/form-data">
